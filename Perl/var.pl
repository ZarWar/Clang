...
04.10.2020_____________________________________________________________
ctrl+D - End of File.

НААУЧИСЬ РАЗВОРАЧИВАТЬ МАССИВ ОДНОЙ СТРОКОЙ!!
@arr = (1, 2, 3, 4, 5);
for (my $i = 0; $i<@arr2/2; $i++) {
        my $t  =  $arr2[@arr2-$i];
        $arr[@arr2-$i]=$arr[$i];
        $arr[$i] = $t;
}
 ...
 1 use strict;
  2 use warnings;
  3 
  4 my @arr = qw/ aaa bbb ccc dd /;
  5 
  6 my $count = @arr;
  7 
  8 # чтобы развернутьо массив достаточно поменять местами
  9 # попарные элементы с начала и с конца
 10 
 11 print "@arr\n";
 12 
 13 for ( my $i = 0; $i < @arr / 2; $i++ ) {
 14         my $t = $arr[ @arr - 1 - $i ];
 16         $arr[ @arr - 1 - $i ] = $arr[ $i ];
 18         $arr[ $i ] = $t;
 19 }
 20 
 21 print "@arr\n";
